...
## 2026-08-29 — Todo 11: end-to-end auto-dictionary proof

- Integration-test scale is `compression_sample_window = 409600` and `compression_dictionary_max_size = 4096`, exactly satisfying config.rs's enforced 100x ratio. Before socket wiring, `train_dictionary` was empirically run against both the train.rs-style seven-sample JSON corpus and the sampler-like 25 x 16 KiB JSON corpus; both produced a non-empty dictionary in under one second.
- The real tunnel test sends 32 x 16 KiB structured payloads through an echo visitor held open before training. The server log reported `sample_bytes=409600`, `dict_bytes=677`, and the exact `trained compression dictionary` transition about 0.2 seconds after the sampling window filled on the local test host.
