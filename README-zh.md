...
```toml
[client]
remote_addr = "example.com:2333" # 必要。服务器地址。也可以是数组，同时连接多个配置一致的服务端，并把本地所有 reverse proxy 注册到每一台上，用于高可用 / 出口负载均衡：["server1:2333", "server2:2333"]
...
compression_level = 9 # Optional. zstd 级别 1..=19，高于 19 会打印警告并改用 9。Default: 在 `compression = "zstd"` 时为 9。仅服务端可配置，作用于本进程编码器。不必为此升级客户端。参见 `docs/compression.md`
